...
const Dashboard = () => {
  ...
  //check for error
    ...
    return (
      <StatusMessage type="loading" message={error?.response?.data?.message} />
    );
  } else {
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg">
            ...
            <div>
              ...
              <p className="mb-4">Credit Remaining: {creditRemaining}</p>
              <p className="mb-4">
                Next Billing Date:{" "}
                {data?.user?.nextBillingDate ?? "No billing date available"}
              </p>
            </div>
          </div>
          ...
          <div className="mb-6 bg-white p-4 shadow rounded-lg col-span-1 md:col-span-2">
            ...
            {data?.user?.payments?.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                ...
                {data?.user?.payments.map((payments) => {
                  const { subscriptionPlan, createdAt, status, amount } =
                    payments;
                  return (
